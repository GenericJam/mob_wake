...
First Hex release. Ships the full four-trigger dispatch surface (iOS
BGTaskScheduler, iOS silent APNs, Android WorkManager, Android FCM
data messages), verified end-to-end on a Moto G Power 5G 2024 and an
iPhone SE 3rd-gen (MOB-268, MOB-271).
...
### Pre-history — scaffold (2026-09-18, git-only, never on Hex)

Initial commit landed the mix.exs / manifest / Elixir dispatch surface only; the native trigger paths (MOB-261..264) came in subsequent commits. That scaffold was kept off Hex deliberately until the full four-trigger surface + physical-device verification landed. Everything above is what 0.1.0 actually ships.
